...
describe("list-performance-images rule", () => {
	...
	it("must use expo-image or request resized/thumbnail URL", () => {
		...
		const usesThumbnailUrl = /thumbnailUrl|thumbnail|w=\d+|h=\d+|fit=/.test(source);
	...
	it("must not use raw imageUrl for list thumbnail without resize", () => {
		...
		const rawUriOnly = /source=\{\{\s*uri:\s*product\.imageUrl\s*\}\}/.test(source);
